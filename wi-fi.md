...
1. **Join the setup network**
   - The first five LEDs pulse blue on first boot or after clearing credentials.
...
**Status LEDs (first five)**

   - **Pulsing blue**: credential portal open.
   - **Blue sweep**: device applies the new credentials.
   - **Solid green**: device connected to your Wi-Fi network.
   - **Double red blink**: connection failed; the portal stays open so you can retry.

## Normal operation

   - The controller reconnects on future boots using the saved credentials. To clear them, run `idf.py -p <PORT> erase-flash` and reboot; the pulsing blue LED returns to show AP mode is active.
